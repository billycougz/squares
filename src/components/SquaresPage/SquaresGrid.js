	...
	return (
		<Box>
			<Grid container>
				...
				<Grid sx={{ width: '47px', flexGrow: '0', flexBasis: 'auto' }} />
				<Grid sx={{ flexGrow: 'calc(1/11 + .009)' }} />
				<Grid
					sx={{
						mt: 1,
			...
			<Grid container wrap="nowrap">
				<Grid
					...
					>
						...
						>
							{Array.from(teams.vertical.name).map((letter, index) => (
								<Typography key={index} variant='h6' sx={{ height: letter === ' ' ? '12px' : '' }}>
									{letter}
								</Typography>
							))}
						</Box>
				...
				{gridData.map((values, rowIndex) => (
					<Grid sx={{ minWidth: 0, flexGrow: 1 }} key={rowIndex}>
